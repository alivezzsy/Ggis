...
void WhatIsThisDialog::setupLayout() {
	...
	labelGeomType = new QLabel(this);
	labelLayerName = new QLabel(this);
	SpatialRef =new QLabel(this);
	SpatialRef->setWordWrap(true);
	...
	formLayout->addRow(new QLabel("Spatial Ref:", this), SpatialRef);
...
void WhatIsThisDialog::setFeature(GeoFeatureLayer* layer, GeoFeature* feature) {
	...
	{
		SpatialRef->setText(layer->getSpatialRef());
	}
	else
		SpatialRef->setText(u8"Î´¶¨Òå");
	/* More info */
