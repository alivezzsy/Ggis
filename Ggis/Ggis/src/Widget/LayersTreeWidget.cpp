...
void LayersTreeWidget::createMenus()
{
	...
	menuAddData = new QMenu(tr("Add Data"), this);
}
...
void LayersTreeWidget::createActions()
{
	...
	popMenuOnMap->addMenu(menuAddData);
	//add shp data
	addShpDataAction = new QAction(tr("add shp data"), this);
	menuAddData->addAction(addShpDataAction);
	connect(addShpDataAction, &QAction::triggered,
		AppEvent::getInstance(),&AppEvent::onAddShpData);
	//add json data
	addJsonDataAction = new QAction(tr("add json data"), this);
	menuAddData->addAction(addJsonDataAction);
	connect(addJsonDataAction, &QAction::triggered,
		AppEvent::getInstance(), &AppEvent::onAddJsonData);
...
void LayersTreeWidget::onShowStyleDialog()
{
	...
	if (layerItem) {
		...
		if (layer && layer->getLayerType() == kFeatureLayer) {
			...
			styleDialog->setFixedSize(550, 450);
			styleDialog->show();
		}
	}
}
